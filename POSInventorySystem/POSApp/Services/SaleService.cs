    ...
    public class SaleService
    {
        ...
        public DataTable GetAvailableProducts()
        {
            return _dbHelper.ExecuteQuery(@"
                SELECT p.ProductID, p.ProductName, p.Price, p.StockQuantity, p.Barcode, p.UnitType, p.DiscountRate, p.IsBOGO, p.CategoryID, t.TaxPercentage
                FROM Products p
        ...
        {
            string query = @"
                SELECT p.ProductID, p.ProductName, p.Price, p.StockQuantity, p.UnitType, p.DiscountRate, p.IsBOGO, p.CategoryID, t.TaxPercentage
                FROM Products p
                LEFT JOIN TaxCategories t ON p.TaxCategoryID = t.TaxCategoryID
